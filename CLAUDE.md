...
`nginx/nginx.conf` proxies for `nerdifica.com`/`www.nerdifica.com` and terminates TLS on 443, redirecting 80 → 443 (except for the ACME challenge path). Certificates come from Let's Encrypt, and the whole chain is automatic, wired into `deploy.yml`: `cert-init` (a one-shot service in `docker-compose.yml`) guarantees nginx always has *some* cert to start with (dummy, self-signed) even before a real one exists; `nginx`'s own entrypoint polls for the cert file before actually starting, since `depends_on` completion conditions aren't reliably honored by every Docker Compose version; `ensure-cert.sh` runs on every deploy and requests a real cert via `--keep-until-expiring`, which no-ops locally unless one doesn't exist yet or is close to expiry (safe against Let's Encrypt's rate limits — never change this to `--force-renewal` for routine use); the `certbot` service then renews in the background and `nginx` reloads periodically to pick up renewals. `CERTBOT_EMAIL` comes from a GitHub Actions secret in the deploy workflow (see `.env.example` for the manual-run fallback). Changing the domain means editing `nginx/nginx.conf`, the `domains=(...)` list in `ensure-cert.sh`, and the two `nerdifica.com` paths hardcoded in `cert-init`/`nginx`'s entrypoints in `docker-compose.yml`.
